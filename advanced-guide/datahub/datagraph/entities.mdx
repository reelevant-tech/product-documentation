...
<Info>
  When the Entity belongs to a Live [Datagraph Entity Group](/advanced-guide/datahub/datagraph/entity-groups), publishing fails with *Entity group contract violated* if the new output columns no longer satisfy the group contract. An Entity listed in a Live group cannot be deleted either.
</Info>

## Sharing a Contract Across Entities

Several Entities that answer the same question for different segments — one recommendation algorithm per Entity, for example — can be bound to a Datagraph Entity Group. The group declares the fields every member must return, and checks the Live version of each member against it. See [Datagraph Entity Groups](/advanced-guide/datahub/datagraph/entity-groups).
...
<CardGroup cols={2}>
  <Card title="Entity Groups" icon="layer-group" href="/advanced-guide/datahub/datagraph/entity-groups">
    Bind several Entities to one shared output contract.
  </Card>
