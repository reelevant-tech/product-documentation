...
Over SFTP, every file pushed within the same session is processed in a single update rather than one update per file. Processing starts when the session is closed, or after about one minute without a new upload if the session stays open.
