...
### Pushing Several Files in One Session

Over both SFTP and FTP, files pushed back-to-back within the same session (connection) are grouped into a single update rather than one update per file. A group is processed when the session is closed, or after about one minute without a new upload if the session stays open. Files pushed later in a still-open session form the next group.
...
<Tabs>
  ...
  <Tab title="FTP">
    ...
    With a graphical FTP client, keep a single connection open and upload the files one after the other. Clients that open several parallel connections to transfer files create one group per connection.
  </Tab>
</Tabs>

<Info>
  If the connection drops while a file is still being transferred, that file is discarded and never processed. Files fully transferred earlier in the same session are still processed as usual.
</Info>
