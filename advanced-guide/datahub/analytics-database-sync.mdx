...
The example below configures a single BigQuery destination on the Ingester Datasource `6718ad69056acb425fbecf1a` (replace the bearer token with your access token — see [Authentication](/developer-docs/api-reference/authentication)):
...
On a Worker Datasource, the same destination is sent as a top-level `destinations` field instead:
