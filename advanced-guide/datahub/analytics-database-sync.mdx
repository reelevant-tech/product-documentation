...
Destinations are set through the Datasource's `patch` step. The payload depends on the Datasource mode:

| Mode | Payload field |
|------|---------------|
| **Ingester**, **Pubsub** | `storageOptions.destinations` |
| **Worker** (including purchases Datasources) | `destinations` |

The example below configures a single BigQuery destination on the Ingester Datasource `6718ad69056acb0300becf1a` (replace the bearer token with your access token — see [Authentication](/developer-docs/api-reference/authentication)):

```bash
curl -XPOST https://api.reelevant.com/v2/datasources/6718ad69056acb0300becf1a/steps \
...
On a Worker Datasource — here `68f21c4d5859000100b3c7a2` — the same destination is sent as a top-level `destinations` field instead:

```bash
curl -XPOST https://api.reelevant.com/v2/datasources/68f21c4d5859000100b3c7a2/steps \
  -H "Authorization: Bearer ${access_token}" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "patch",
      ...
      "destinations": [
        {
          "type": "bigquery",
          "dataset": "tracking_events_production_eu",
          "tableName": "datasources_purchases_{COMPANY_ID}",
          "deduplicateOptions": { "enabled": true }
        }
      ]
    }
...
<Warning>
  Sending `storageOptions` to a Worker Datasource — or `destinations` to an Ingester or Pubsub Datasource — is silently ignored: the request returns a success response and the configuration stays unchanged. Read the Datasource back after the `patch` step to confirm the destination was saved.
</Warning>
