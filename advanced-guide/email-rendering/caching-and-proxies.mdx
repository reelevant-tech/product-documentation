...
|---|---|---|---|
| Gmail image proxy | Yes | Routes images through Google's proxy and caches them, even though the Runner returns no-cache headers. | `GmailImageProxy` in the user agent. |
...
| Outlook web | Often | Webmail that can serve images through a caching proxy. | `Outlook` user agent, or an `outlook`/`office`/`live`.com referer. |
...
That encourages clients that honour cache headers to request a fresh image. Proxy-based clients such as Gmail and Apple Mail can still cache the image despite these headers.
