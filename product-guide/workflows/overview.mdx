...
1. **Where** does the content appear? ([Channels](/platform-guide/workflows/channels))
2. **What data** drives the personalization? ([Data nodes](/platform-guide/workflows/data-nodes))
3. **What logic** determines the experience? ([Logic nodes](/platform-guide/workflows/logic-nodes), [conditions](/platform-guide/workflows/logic-nodes/conditions), [splits](/platform-guide/workflows/logic-nodes/split))
...
<CardGroup cols={2}>
  ...
  <Card title="Logic Nodes" icon="code-branch" href="/platform-guide/workflows/logic-nodes">
    Control the flow — conditions, A/B splits, and real-time testing decide which branch a user follows.
  </Card>
  <Card title="Output Nodes" icon="square-check" href="/platform-guide/workflows/output-nodes">
    Define what gets returned — a visual content template, a JSON template payload, or an empty response.
  </Card>
</CardGroup>

Nodes are connected by **edges** that define the flow of execution from [channels](/platform-guide/workflows/channels) through [data](/platform-guide/workflows/data-nodes) and [logic](/platform-guide/workflows/logic-nodes) nodes down to [outputs](/platform-guide/workflows/output-nodes).
...
A **branch** is a unique path through the workflow from a [channel](/platform-guide/workflows/channels) to an [output node](/platform-guide/workflows/output-nodes). Branches are the fundamental unit of personalization — each branch can deliver a different experience based on the user's context.
