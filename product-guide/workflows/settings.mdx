...
description: 'Configure variable profiles and JSON templates for your workflows'
---
...
<Note>
  Branch categories are configured together with tags in [Analytics Settings](/product-guide/analytics/settings), so your whole nomenclature lives in one place.
</Note>
...
<Steps>
  ...
  <Step title="Choose a click redirect variable (optional)">
    Use the **Click redirect variable** dropdown to pick the variable that holds a URL (e.g., `productUrl`). When a visitor clicks the Workflow's click tracking link, they are redirected to that URL and the click is recorded.

    Only **Scalar** and **Datasource item** variables can be selected. Leave **No redirect** if the template is not meant to be clicked.
  </Step>
  <Step title="Save">
...
Click on a template in the list to open its edit drawer. You can update the name, JSON definition, variables, and click redirect variable. Changes take effect on the **next publish** of any workflow using this template.
