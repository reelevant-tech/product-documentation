...
<Steps>
  ...
  <Step title="Choose a click redirect variable (optional)">
    Use the **Click redirect variable** dropdown to pick the variable that holds a URL (e.g., `productUrl`). When the workflow is called in click mode, it redirects the visitor to that URL and records the click.
