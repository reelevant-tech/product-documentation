...
The debugging view is protected and only opens with a valid debug token. The extension handles this for you: while you are logged in, the token is attached automatically, so you never need to copy it by hand.

<Info>
  If the debugging view answers with an error, check that you are still logged in to the extension, then reload the page.
</Info>
