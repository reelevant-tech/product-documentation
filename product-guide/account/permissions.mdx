...
The Reelevant platform uses a **role-based access control** (RBAC) system. Every user is assigned exactly one [role](/platform-guide/account/roles) that defines what they can do and which resources they can interact with. Resources are further scoped by [teams](/platform-guide/account/teams) so that users only see what belongs to their teams.
...
|----------|-------------|---------|
| **Workflow** | [Workflow](/platform-guide/workflows/overview) creation and management. | Company + Team |
| **Workflow Settings** | Global workflow configuration (variable profiles, etc.). | Company |
| **Content** | [Content template](/platform-guide/contents/overview) creation and management. | Company + Team |
| **Datasource** | [Datasource](/platform-guide/datahub/overview) creation and management. | Company + Team |
| **Statistics** | [Analytics](/platform-guide/analytics/overview) data access and export. | Company |
...
The table below shows which actions are available for each resource. When configuring a [role](/platform-guide/account/roles), you can toggle each available action individually.
...
<Steps>
  ...
  <Step title="Team check (team-scoped resources only)">
    Is the user a member of at least one team that the target resource belongs to? Team membership includes inherited access from the [team hierarchy](/platform-guide/account/teams#team-hierarchy).
  </Step>
</Steps>
