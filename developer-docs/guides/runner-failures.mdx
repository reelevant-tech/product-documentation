...
| No destination URL, JSON or JSON Template Output Node | `204`, empty body. |
| No destination URL, any other Output Node | `404`. |

Set `rlvt-redirect` to override the destination resolved by the Workflow.

A JSON Template Output Node resolves its destination from the template's `redirectVariable`: the click redirects to the value bound to that variable (a scalar or fixed-position variable holding a URL). Templates without `redirectVariable` return `204` unless `rlvt-redirect` is set. See [JSON Templates settings](/product-guide/workflows/settings#json-templates).
...
### Automatic injection by the browser extension

The [browser extension](/product-guide/browser-extension/overview) mints this token itself and injects `x-rlvt-debug-token` on every top-level and iframe navigation to `reelevant.run` while you are logged in. Opening `mode=debug` or `mode=ui-debug` from the extension toolbox, or pasting such a URL in the address bar, therefore needs no manual token.

- The token is minted with the extension session, so it carries the same company scope as your account (platform admins get a wildcard token).
- It is refreshed at most every 5 minutes, well under the 15-minute TTL, and the header is removed on logout.
- The Runner only reads the header on `mode=debug` / `mode=ui-debug`, so it is inert on regular requests.
- Platform admins can paste a **Custom debug token** in the extension settings to override the minted one.
...
| Code | Meaning |
|---|---|
| _empty_ | The run completed without error. |
...
| `3503` | Click run with no destination URL on a non-JSON Output Node — the run answered `404`. JSON and JSON Template Output Nodes answer `204` without emitting a code. |
