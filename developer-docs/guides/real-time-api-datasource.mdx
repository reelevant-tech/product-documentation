...
<Warning>
  The upstream host must be publicly reachable. Requests that resolve to private or internal IP ranges are rejected with `UrlNotReachable`, both when configuring the source and on every live query. If the host allowlists inbound traffic, add the [Reelevant public IPs](/advanced-guide/datahub/public-ips).
</Warning>
...
- [Public IPs](/advanced-guide/datahub/public-ips) — CIDRs to allowlist on the upstream host.
