...
You do not need a Workflow to verify the Datasource — call the [Query a datasource](/api-reference/datasource/query-a-datasource) endpoint directly. The `query` field is a JSON-encoded filter; variables and response fields are filtered exactly as in a Data Node. Pass pagination as query parameters.
...
<Tip>
  Send an empty query (`"query": "{}"`) to fetch with the variables' `default` values, or add the `x-rlvt-no-cache: true` header to bypass the cache window configured in [step 5](#5-configure-caching-optional) while testing.
</Tip>
