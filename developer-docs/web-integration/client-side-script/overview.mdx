...
Client-side personalisation uses a JavaScript tracking script that loads in the browser. It fetches personalised content after the page renders and injects it into designated DOM zones. This is the original integration method and works alongside the [Server Side SDK](/developer-docs/web-integration/server-side-sdk/overview).
...
|----------|---------------------|
| Content that must be visible immediately (no flash) | [Server-side SDK](/developer-docs/web-integration/server-side-sdk/overview) |
| SEO-critical personalised content | [Server-side SDK](/developer-docs/web-integration/server-side-sdk/overview) |
...
Add the Reelevant tracking script to your page. The integration URL is generated from the [Integration modal](/product-guide/workflows/integration) in the workflow editor.
...
These values are automatically forwarded to workflows as URL parameters for use in [data nodes](/product-guide/workflows/data-nodes/url-parameter).
