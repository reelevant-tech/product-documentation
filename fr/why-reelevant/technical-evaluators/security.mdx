...
description: "Attestation SOC 2 Type 2, conformité RGPD, standards de chiffrement, politique de mot de passe, gestion des vulnérabilités, fournisseurs et posture de sécurité de Reelevant pour les évaluateurs techniques."
keywords: ["SOC 2", "RGPD", "politique de mot de passe", "logs d'audit", "journal d'audit", "provisionnement SCIM", "sous-traitants", "fournisseurs", "contact sécurité", "divulgation de vulnérabilité", "DPA", "notification de violation", "suppression des données", "pseudonymisation", "questionnaire de sécurité"]
---
...
|----------|--------|--------|
| **SOC 2 Type 2** | Attesté | Audit indépendant de la conception et de l'efficacité opérationnelle de nos contrôles au regard de la catégorie Sécurité des Trust Services Criteria, reconduit annuellement. Le périmètre est la plateforme Reelevant ; les hébergeurs sont couverts par leurs propres attestations |
...
| **Audit automatisé continu** | En continu | Analyse automatisée du code, des dépendances et de la configuration cloud à chaque modification |
| **Certification ISO 27001 / article 42** | Non détenue | La conformité est démontrée par l'attestation SOC 2 Type 2, le DPA et les mesures décrites sur cette page |

Le rapport SOC 2 Type 2 en vigueur, ceux de nos hébergeurs, ainsi que la synthèse du dernier test d'intrusion sont disponibles via le **[Trust Centre](https://trust.reelevant.com)** sous NDA.
...
| Donnée | Origine | Remarques |
|--------|---------|-----------|
| Champs des Datasources | Synchronisés depuis vos systèmes | Uniquement les champs que vous mappez ; les données de contact sont remplacées à chaque synchronisation, un enregistrement retiré à la source n'est donc plus traité |
| Identifiant client pseudonyme | Transmis dans les URL du Runner par votre outil d'emailing ou votre site | Un identifiant opaque issu de vos systèmes — jamais une adresse email ni une autre donnée directement identifiante |
| Événements comportementaux | Générés par le Runner (expositions, clics, Branche exécutée) et par le tag web (pages vues) | Rattachés à l'identifiant pseudonyme ; conservés 90 jours par défaut |
| Conversions attribuées et agrégats | Calculés par la plateforme | Les agrégats (Uplift, Incremental Value, rapports) ne sont pas des données personnelles |
...
- Effacement à l'enregistrement disponible sur demande via l'[opération d'anonymisation des Datasources](/fr/advanced-guide/datahub/anonymise-customer-records)

### Engagements contractuels

Les engagements suivants font partie du contrat de sous-traitance (DPA) standard :

| Engagement | Condition standard |
|------------|--------------------|
| Lieu de traitement | UE uniquement, aucun transfert hors UE |
| Notification de violation de données personnelles | Sous 48 heures après détection, via security@reelevant.com |
| Fin de contrat | Suppression sécurisée de l'ensemble des données personnelles (bases de données, caches, sauvegardes) dans les 30 jours suivant la fin du contrat |
| Restitution des données | Export dans un format structuré sur demande écrite avant suppression |
| Sauvegardes | Les sauvegardes chiffrées expirent au plus tard 180 jours après la suppression |
| Preuve de suppression | Suppression tracée et attestation écrite fournie sur demande |
| Accès du personnel à vos données | Limité au support, au diagnostic à votre demande et à la maintenance opérationnelle — sous moindre privilège, MFA et journalisation |
...
| Contrôle | Par défaut |
|----------|------------|
| Longueur minimale | 12 caractères, dont au moins un chiffre et un caractère spécial |
| Expiration | 6 mois, puis renouvellement exigé (désactivable) |
| Réutilisation | Le mot de passe courant et les 3 précédents sont refusés |
| Tentatives échouées | Compte verrouillé 10 minutes après 10 tentatives échouées |
| Stockage | Haché avec bcrypt et un sel unique — jamais stocké en clair ni journalisé |
| Réinitialisation | Jeton à usage unique envoyé à l'adresse e-mail vérifiée du compte |
...
| **AWS** | Route 53 (DNS) | Global (anycast, comme l'exige le DNS) | Résolution DNS publique des domaines Reelevant — aucune donnée client, donc pas un sous-traitant |
...
| Sous-traitant | Entité juridique | Traitement | Localisation |
|---------------|------------------|------------|--------------|
| OVHcloud | OVH SAS, 2 rue Kellermann, 59100 Roubaix, France — RCS Lille Métropole 424 761 419 | Hébergement de la plateforme sur serveurs dédiés : services applicatifs, bases de données, traitement des événements, sauvegardes | France (UE) |
| Google Cloud | Google Cloud EMEA Limited, Velasco, Clanwilliam Place, Dublin 2, Irlande | Hébergement de l'entrepôt analytique (BigQuery) contenant les événements pseudonymes et les agrégats | Belgique, région `europe-west1` (UE) |

Aucun de ces sous-traitants n'accède aux données clients à distance depuis l'extérieur de l'UE.

## Journalisation des audits

- **[Journal d'audit](/fr/product-guide/account/audit-log) dans la plateforme** — authentification, SSO, provisionnement SCIM et modifications des utilisateurs, des rôles et des Teams, consultable par vos propres administrateurs d'identités
