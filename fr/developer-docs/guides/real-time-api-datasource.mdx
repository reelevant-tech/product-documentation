...
Ce guide construit une Datasource proxy de bout en bout avec `curl`, à partir d'une fausse API produits. Chaque appel cible l'API datasources sur `https://api.reelevant.com/v2` et nécessite un token d'accès — voir [Authentification](/fr/developer-docs/api-reference/authentication) pour savoir comment l'obtenir.
