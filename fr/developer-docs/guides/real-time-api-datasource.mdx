...
<Warning>
  L'hôte externe doit être joignable publiquement. Les requêtes qui se résolvent vers des plages d'IP privées ou internes sont rejetées avec `UrlNotReachable`, à la fois lors de la configuration de la source et à chaque requête en direct. Si l'hôte autorise le trafic entrant par IP, ajoutez les [IPs publiques Reelevant](/fr/advanced-guide/datahub/public-ips).
</Warning>
...
- [IPs publiques](/fr/advanced-guide/datahub/public-ips) — CIDR à autoriser sur l'hôte amont.
