...
| Un Node a échoué mais la branche s'est terminée (erreur silencieuse) | `200`, sortie normale. |
...
`mode=click` résout l'URL de destination de la branche exécutée et redirige vers celle-ci.
...
| Aucune URL de destination, Output Node JSON ou JSON Template | `204`, corps vide. |
...
Un Output Node JSON Template résout sa destination à partir du `redirectVariable` du template : le clic redirige vers la valeur liée à cette variable (une variable scalaire ou à position fixe contenant une URL). Les templates sans `redirectVariable` renvoient `204` sauf si `rlvt-redirect` est défini. Voir [paramètres des JSON Templates](/fr/product-guide/workflows/settings#json-templates).
...
### Injection automatique par l'extension navigateur

L'[extension navigateur](/fr/product-guide/browser-extension/overview) génère elle-même ce token et injecte `x-rlvt-debug-token` sur chaque navigation de premier niveau ou d'iframe vers `reelevant.run` tant que vous êtes connecté. Ouvrir `mode=debug` ou `mode=ui-debug` depuis la boîte à outils de l'extension, ou coller une telle URL dans la barre d'adresse, ne nécessite donc aucun token manuel.

- Le token est généré avec la session de l'extension : il porte le même périmètre société que votre compte (les admins plateforme obtiennent un token wildcard).
- Il est rafraîchi au plus toutes les 5 minutes, bien en dessous du TTL de 15 minutes, et l'en-tête est retiré à la déconnexion.
- Le Runner ne lit l'en-tête que sur `mode=debug` / `mode=ui-debug` : il est inerte sur les requêtes classiques.
- Les admins plateforme peuvent coller un **Jeton de débogage personnalisé** dans les paramètres de l'extension pour remplacer celui généré automatiquement.
...
| `3502` | Erreur silencieuse : au moins un Node a échoué mais la branche s'est terminée. Le contenu a été servi. |
| `3503` | Exécution en mode click sans URL de destination sur un Output Node non JSON — la réponse a été un `404`. Les Output Nodes JSON et JSON Template répondent `204` sans émettre de code. |
