...
3. **Team** — Pour les ressources délimitées par Team, l'utilisateur ne peut accéder qu'aux ressources appartenant à l'un de ses Teams. Chaque permission du rôle peut élargir cette portée aux Teams parents — voir [portée de Team](#team-scope).
...
Le tableau ci-dessous indique quelles actions sont disponibles pour chaque ressource. Lors de la configuration d'un [rôle](/fr/product-guide/account/roles), vous réglez chaque action disponible individuellement, ainsi que sa [portée de Team](#team-scope).
...
<Steps>
  ...
  <Step title="Vérification du Team (ressources délimitées par Team uniquement)">
    L'utilisateur est-il membre d'au moins un Team auquel appartient la ressource ciblée ? L'appartenance à un Team inclut l'accès hérité de la [hiérarchie des Teams](/fr/product-guide/account/teams#team-hierarchy), élargi par la [portée de Team](#team-scope) de la permission.
  </Step>
</Steps>
...
Chaque permission sur une ressource délimitée par Team porte l'une des deux portées de Team, choisie dans l'[éditeur de rôles](/fr/product-guide/account/roles#team-scope-per-permission) :

| Portée de Team | Ressources accessibles à l'utilisateur |
|----------------|----------------------------------------|
| **Own teams** | Les ressources des Teams de l'utilisateur et de leurs sous-Teams, plus celles des Teams parents sauf si un Team est [exclu de l'héritage ascendant](/fr/product-guide/account/teams#exclude-from-ancestor-inheritance). C'est la portée par défaut. |
| **Own teams and their parent teams** | Tout ce qui précède, plus les ressources de chaque Team parent, même lorsqu'un Team est exclu de l'héritage ascendant. |

La portée se règle par ressource **et** par action. Sur un Team exclu de l'héritage ascendant, un rôle peut donc rouvrir les Datasources des Teams parents tout en limitant les Workflows aux Teams de l'utilisateur.
