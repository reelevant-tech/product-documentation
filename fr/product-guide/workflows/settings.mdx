...
description: 'Configurer les Variable Profiles et les JSON Templates pour vos Workflows'
---
...
<Note>
  Les catégories de branche se configurent avec les tags dans les [paramètres Analytics](/fr/product-guide/analytics/settings), pour regrouper toute votre nomenclature au même endroit.
</Note>
...
<Steps>
  ...
  <Step title="Choisir une variable de redirection au clic (optionnel)">
    Utilisez le menu **Variable de redirection au clic** pour choisir la variable qui contient une URL (ex. `productUrl`). Lorsqu'un visiteur clique sur le lien de suivi des clics du Workflow, il est redirigé vers cette URL et le clic est enregistré.

    Seules les variables **Scalar** et **Datasource item** peuvent être sélectionnées. Laissez **Aucune redirection** si le template n'a pas vocation à être cliqué.
  </Step>
  <Step title="Enregistrer">
...
Cliquez sur un template dans la liste pour ouvrir son panneau d'édition. Vous pouvez mettre à jour le nom, la définition JSON, les variables et la variable de redirection au clic. Les modifications prennent effet à la **prochaine publication** de tout Workflow utilisant ce template.
...
2. **Cohérence inter-branches** — Tous les Nodes JSON Template de toutes les branches doivent référencer le **même template**. Cela garantit que chaque branche produit la même forme JSON.

Si la validation échoue, la publication est bloquée avec un message d'erreur clair indiquant quel Node ou quelle branche présente une incohérence.
