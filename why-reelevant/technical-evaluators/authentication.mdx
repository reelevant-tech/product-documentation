...
keywords: ["SSO", "SAML 2.0", "SCIM 2.0", "JIT provisioning", "user provisioning", "Okta", "Microsoft Entra ID", "OAuth 2.0", "RBAC", "ABAC"]
---
...
| Attribute mapping | Map IdP assertion attributes to Reelevant user fields (email, first name, last name, job title, phone, preferred language) |
| Default role | Role assigned to users created via the platform |
| User mode | `invitation` (default) or `create` — see JIT provisioning below |
...
| **JIT on SSO login** | The account is created on first successful SSO login, if SSO user mode is `create` | Manual |
...
| Mode | Behaviour |
|------|-----------|
| `invitation` (default) | The login is rejected unless the address already has an account or a pending invitation. Use this to keep the account list closed. |
| `create` | The account is created from the assertion — just-in-time provisioning. No invitation needed. |

A JIT-created user gets its profile from your attribute mapping, and its role and Teams from the pending invitation when there is one, otherwise from the company defaults. An invitation's role and Teams are re-validated against the company before they are honoured. Creation is recorded in the audit log as an `sso` event attributed to `sso-jit`.
