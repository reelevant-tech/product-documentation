...
| **Query parameters** | URL query parameters to append to the request (key-value pairs). |
...
The URL source supports fetching **PGP-encrypted files**. When a PGP private key is configured, the response is decrypted before decompression and parsing.
...
<Tabs>
  <Tab title="Query Parameter">
    ...
    | Field | Description |
    |-------|-------------|
    | **Parameter name** | The query parameter name (e.g. `page`, `offset`). |
    | **Start at** | The initial value (default: `1`). Some APIs start at `0`. |
  ...
  <Tab title="Hypermedia Attribute">
    ...
    | Field | Description |
    |-------|-------------|
    | **Attribute** | The JSON path to the next page URL in the response (e.g. `next_page_url`). |
  </Tab>
  <Tab title="Page Token">
    ...
    | Field | Description |
    |-------|-------------|
    | **Token attribute** | The JSON path to the token in the response body. |
    | **Query parameter** | The query parameter to use for the token in subsequent requests. |
  </Tab>
  ...
  <Tab title="Entry Token">
    ...
    | Field | Description |
    |-------|-------------|
    | **Token attribute** | The field name in each entry that serves as a cursor. |
    | **Query parameter** | The query parameter to use for the cursor in subsequent requests. |
  </Tab>
</Tabs>

<Info>
  You can optionally set a **Maximum page** limit to cap the number of pages fetched during a sync.
</Info>
...
| **Default value** | A static value or a dynamic expression evaluated at runtime. |
