...
const response = await fetch(
  `https://reelevant.run/workflows/run/${workflowId}/0?rlvt-u=${encodeURIComponent(userId)}`
)
...
response = requests.get(
    f"https://reelevant.run/workflows/run/{workflow_id}/0",
...
```bash
curl -s "https://reelevant.run/workflows/run/{workflowId}/0?rlvt-u=user@example.com"
```
