...
Suppose you want to personalise a product recommendation card. Your JSON Template schema might look like:
...
At runtime, the Reelevant engine resolves each variable from your datasources — so each visitor sees the product most relevant to them. Your frontend simply renders the JSON fields using its own components.

## Next steps

<CardGroup cols={2}>
  <Card title="Technical Integration" icon="code" href="/platform-guide/omni-channels/websites/api-json/integration">
    API endpoints, request format, response structure, and code examples.
  </Card>
  <Card title="JSON Template (Output Node)" icon="file-code" href="/platform-guide/workflows/output-nodes/json-format">
