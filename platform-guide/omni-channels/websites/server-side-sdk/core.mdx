...
Every `RunResult` includes a `redirectionUrl` — a pre-built runner URL with `mode=click`. When visited, the runner records the click and 302-redirects the user to the final destination. This ensures click tracking is always recorded. You must use either the redirect link pattern or the `trackClick()` callback.
...
Use `client.trackClick()` when you want to track the click server-side without redirecting through the runner. The runner records the click and the response is discarded.
