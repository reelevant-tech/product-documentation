...
Every `RunResult` includes `redirectionUrl` (runner URL with `mode=click`) and `clickUrl` (final destination). Use `redirectionUrl` as a redirect link or track clicks server-side via `trackClick`:
...
// trackClick is available from useReelevant()
export default defineEventHandler(async (event) => {
  const { run, trackClick } = useReelevant({ client: rlvt, event })
  const result = await run({ workflowId: 'wf-hero', entrypoint: 'hero' })

  // Later, when the user clicks:
  await trackClick({ redirectionUrl: result.redirectionUrl })
  // Navigate user to result.clickUrl yourself
})
```
