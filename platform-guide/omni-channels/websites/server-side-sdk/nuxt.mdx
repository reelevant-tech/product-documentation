...
Every `RunResult` includes a `clickUrl`. Use it as a redirect link or track clicks server-side:
...
<template>
  <div v-if="data?.hero?.body?.type === 'html'" data-rlvt-ssr="true">
    ...
    <a :href="data.hero.clickUrl">Shop now</a>
  </div>
</template>
```
...
export default defineEventHandler(async (event) => {
  const { clickUrl } = await readBody(event)
