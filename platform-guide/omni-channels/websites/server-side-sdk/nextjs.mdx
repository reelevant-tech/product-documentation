...
Every `RunResult` includes a `clickUrl` for tracking CTA clicks. Two patterns:

### Redirect link

Use the `clickUrl` directly as an `<a href>`:
...
Track the click from a Server Action or API route without redirecting through the runner:
...
export async function trackClick(clickUrl: string) {
  await rlvt.trackClick({ clickUrl })
}
```
