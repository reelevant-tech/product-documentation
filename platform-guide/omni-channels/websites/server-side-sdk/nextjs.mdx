...
Every `RunResult` includes `redirectionUrl` (runner URL with `mode=click`) and `clickUrl` (final destination). Two patterns:
...
Track the click from a Server Action, then navigate to the final destination yourself:
