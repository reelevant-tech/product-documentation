...
Every `RunResult` includes `redirectionUrl` — a runner URL with `mode=click` that ensures clicks are always tracked. Two patterns:
...
export default async function Page() {
  ...
  const result = await rlvt.run({ workflowId: 'wf-promo', entrypoint: 'cta', userId })
...
Track the click from a Server Action — the runner records the click and the response is discarded:
...
export async function trackClick(redirectionUrl: string) {
  await rlvt.trackClick({ redirectionUrl })
}
```
