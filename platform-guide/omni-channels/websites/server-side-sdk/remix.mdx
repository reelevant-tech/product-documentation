...
Every `RunResult` includes `redirectionUrl` (runner URL with `mode=click`) and `clickUrl` (final destination). Use `redirectionUrl` as a redirect link or track clicks via `trackClick` from `createLoader`:
...
// trackClick is available from createLoader()
export async function action({ request }: ActionFunctionArgs) {
  const { trackClick } = createLoader({ client: rlvt, request })
  const { redirectionUrl } = await request.json()
  await trackClick({ redirectionUrl })
  return json({ ok: true })
}
```
