...
Every `RunResult` includes `redirectionUrl` (runner URL with `mode=click`) and `clickUrl` (final destination). Use `redirectionUrl` as a redirect link or track clicks via `trackClick` from `createLoad`:
...
export const actions = {
  trackClick: async (event) => {
    const { trackClick } = createLoad({ client: rlvt, event })
    const formData = await event.request.formData()
    const redirectionUrl = formData.get('redirectionUrl') as string
