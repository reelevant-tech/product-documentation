...
<Steps>
  ...
  <Step title="Select the source type">
    ...
    |-------------|-------------|
    | **Datasource** | Connect to a [DataHub datasource](/platform-guide/datahub/overview) to access its fields. |
...
When a variable is bound, you can apply [transformations](/api-reference/transformations/introduction) to format the value — for example, formatting a date, rounding a number, or converting a currency. See the [Text component](/platform-guide/contents/components/text) for transformation details.
